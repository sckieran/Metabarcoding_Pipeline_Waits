...
bash ${dirr}/scripts/step_4_start.sh -d ${dirr} -n ${prefix} -g ${genelist} -p ${pattern1} -q ${pattern2} -m ${params_file} -l ${rlib} -c ${cutoff} -t ${return_low} -s ${score} -x ${local}  -b ${localdat}
