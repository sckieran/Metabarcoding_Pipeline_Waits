...
do
	...
	echo "${p}	0	No Hit	NA	NA	NA	NA	NA" >> ${prefix}_${gene}_best_blast_hits.out
	else
	num=$(grep  -n "^${p}$" temp_seqlist | awk -F":" '{print $1}')
	echo "doing ${num} (${p}) of ${tot}"
	grep "${p}	" ${blastout}_with_tax | sort -k3 -nr > temp_seq #get all the identities for the hits for a single sequence
	...
	then
		...
		if [[ $spec_num -eq 1 ]]
		then
			spec=$(head -n1 temp_choose |cut -f5 | awk '{print $1}') 
...
#			echo "taxline from multi-hit, one species is $tax"
		...
		do
			...
			#echo "taxid is $taxid"	
			grep "^${taxid}" ${ncbi}_r | cut -f2-6 >> temp_ids
		done < temp_choose
...
do
        ...
        do
                ...
                taxline=$(grep -w "$seq" ${prefix}_${gene}_best_blast_hits.txt | awk -v OFS='\t' '{print $2,$3" "$4,$5,$6,$7,$9,$10}')
