...
while getopts ":n:g:d:r:b:c:t:s:" opt; do
  case $opt in
	...
	blastn -db ${db_dirr}/${localdat} -query ${dirr}/${gene}_dada_out/${prefix}_${gene}_combined_ASVs.fasta -outfmt "6 qseqid sacc pident length stitle ${score}" -culling_limit 3 -num_threads 4 -out ${prefix}_${gene}_raw_blast_out
